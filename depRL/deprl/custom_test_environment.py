...
def test_mujoco(env, agent, steps, params=None, test_episodes=10):
    ...
    for _ in range(test_episodes):
        ...
        while True:
            ...
            if eval_rwd_metrics:
                for k in rwd_metrics.keys():
                    rwd_metrics[k].append(1.0 * env.environments[0].rwd_dict[k])
            if info["resets"][0]:
                break

        for k, v in rwd_metrics.items():
            if k in env.environments[0].rwd_keys_wt:
                rwd_metrics[k] = [env.environments[0].rwd_keys_wt[k] * val for val in v]
        ...
        if eval_rwd_metrics:
            for k, v in rwd_metrics.items():
                metrics["test/rwd_metrics/" + k] = np.sum(v) / metrics["test/episode_length"]
